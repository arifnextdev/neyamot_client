...
export default function SmsPlansClient() {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Choose the perfect SMS plan for your business communication needs
          </p>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {isLoading ? (
            <div className="col-span-full text-center py-12">
              ...
              <p className="mt-4 text-muted-foreground">Loading SMS plans...</p>
            </div>
          ) : (
            data?.products &&
            data.products.map((plan: IProduct) => (
              <PricingCard
                key={plan.id}
                name={plan.name}
                description="Professional SMS Service"
                price={plan.price}
                currency="৳"
